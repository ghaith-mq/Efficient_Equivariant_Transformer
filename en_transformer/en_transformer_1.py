...
from torch import chunk, nn, einsum
from torch.utils.checkpoint import checkpoint_sequential
import os
_folder_current = os.path.dirname(os.path.abspath(__file__))
import sys
sys.path.append(os.path.join(_folder_current, ".."))
...
class EquivariantAttention(nn.Module):
    ...
    def forward(
        ...
        q0, k0, v0 = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = h), (q, k, v))
        chunk_size=8
        # calculate nearest neighbors
        output1 = torch.empty_like(feats)
        coors_output=torch.empty_like(coors)
        rel_coors0=rel_coors
        rel_dist0=rel_dist
        for indx, query in enumerate(q0.split(chunk_size, dim=-2)):  # Here is the main modification, we split the query into chunks and carry on computations per chunk.
            i = chunk_size
            j=n
            if exists(nbhd_indices):
                i, j = nbhd_indices.shape[-2:]
                nbhd_indices_with_heads = repeat(nbhd_indices, 'b n d -> b h n d', h = h)
                k         = batched_index_select(k0, nbhd_indices_with_heads, dim = 2)
                v         = batched_index_select(v0, nbhd_indices_with_heads, dim = 2)
                rel_dist  = batched_index_select(rel_dist0, nbhd_indices, dim = 2)
                rel_coors = batched_index_select(rel_coors0, nbhd_indices, dim = 2)
                rel_dist=rel_dist[:,indx * chunk_size:(indx + 1) * chunk_size,:]
                rel_coors=rel_coors[:,indx * chunk_size:(indx + 1) * chunk_size,:,:]
            else:
                k = repeat(k0, 'b h j d -> b h n j d', n = n)
                v = repeat(v0, 'b h j d -> b h n j d', n = n)
                rel_dist=rel_dist[:,indx * chunk_size:(indx + 1) * chunk_size,:]
                rel_coors=rel_coors[:,indx * chunk_size:(indx + 1) * chunk_size,:,:]
            ...
            if exists(mask):
                ...
                if exists(nbhd_masks):
                    mask0 &= rearrange(nbhd_masks, 'b i j -> b () i j')
            ...
            q_pos_emb0=q_pos_emb
            q_pos_emb0=q_pos_emb0[:,:,indx * chunk_size:(indx + 1) * chunk_size,:]
            k=k[:,:,indx * chunk_size:(indx + 1) * chunk_size,:,:]
            v=v[:,:,indx * chunk_size:(indx + 1) * chunk_size,:,:]
            query = apply_rotary_pos_emb(query, q_pos_emb0)
            k = apply_rotary_pos_emb(k, k_pos_emb)
            v = apply_rotary_pos_emb(v, k_pos_emb)

            # calculate inner product for queries and keys
            qk = einsum('b h i d, b h i j d -> b h i j', query, k)
            #  * (self.scale if not exists(edges) else 1)
            # add edge information and pass through edges MLP if needed

            # if exists(edges):
            #     if exists(nbhd_indices):
            #         edges = batched_index_select(edges, nbhd_indices, dim = 2)
            #     edges=edges[:,indx * chunk_size:(indx + 1) * chunk_size,:,:]
            ...
            if exists(mask):
                mask0=mask0[:,:,indx * chunk_size:(indx + 1) * chunk_size,:]
                mask_value = max_neg_value(coor_weights)
                coor_mask = repeat(mask0, 'b () i j -> b i j ()')
            ...
            if exists(mask):
                ...
                sim.masked_fill_(~mask0, mask_value)
            ...
            output1[:,indx * chunk_size:(indx + 1) * chunk_size,:]=out
            coors_output[:,indx * chunk_size:(indx + 1) * chunk_size,:]=coors_out
        output1 = self.to_out(output1)
        return output1, coors_output
